...
def get_jobs():  # задача Получение всех работ
...
def get_one_job(job_id):  # задача Получение одной работы
    db_sess = db_session.create_session()
    job = db_sess.query(Jobs).get(job_id)
    if not job:
        return make_response(jsonify({'error': 'Not found'}), 404)
    return jsonify({f'job {job_id}': job.to_dict(only=('id', 'team_leader', 'job', 'work_size',
                                                       'collaborators', 'start_date', 'end_date', 'is_finished'))})
